  ...
  if (state === "launch") {
    ...
    enemies.forEach((e, i) => {
      ...
      bullets.forEach((b, bi) => {
        if (
          b.x < e.x + e.width && b.x + 6 > e.x &&
          b.y < e.y + e.height && b.y + 6 > e.y
        ) {
          // kill enemy & bullet
          enemies.splice(i, 1);
          bullets.splice(bi, 1);
...
function showHighScores() {
  ...
  // if we’re in the top ten and initials not yet set
  ...
  ctx.fillText("HIGH SCORES", 320, 150);
  top10.forEach((s, idx) => {
    ctx.fillText(`${idx+1}. ${s.initials || "---"}   ${s.score}`, 300, 190 + idx*30);
  });
}
